 ...
 */
public class RemarkStrategy extends RetrievalStrategy {
    @Override
    public Contactable get(Iterator<? extends Contactable> iterator, String key) {
        while (iterator.hasNext()) {
            Contactable contactable = iterator.next();
            if (Objects.equals(key, contactable.getRemark())) return contactable;
        }
        return null;
    }
    ...
    public boolean remove(Iterator<? extends Contactable> iterator, String key) {
        while (iterator.hasNext()) {
            if (Objects.equals(key, iterator.next().getRemark())) {
                iterator.remove();
                return true;
            }
        }
    ...
    @Override
    public void init() {
        MAP.put(RetrievalType.REMARK_NAME, this);
    }
}
